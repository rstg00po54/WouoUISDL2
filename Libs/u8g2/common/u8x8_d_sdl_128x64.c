...
static void u8g_sdl_set_pixel(int x, int y, int idx)
{
    ...
    //   SDL_UpdatePos(u8g_sdl_screen, x, y, idx);
    for (i = 0; i < u8g_sdl_multiple; i++)
        for (j = 0; j < u8g_sdl_multiple; j++)
        {
            offset =
                (y * u8g_sdl_multiple + j) * width * BytesPerPixel +
                (x * u8g_sdl_multiple + i) * BytesPerPixel;
...
static void u8g_sdl_init(int width, int height)
{
    ...
    //   u8g_sdl_screen = SDL_SetVideoMode(u8g_sdl_width*u8g_sdl_multiple,u8g_sdl_height*u8g_sdl_multiple,32,SDL_SWSURFACE);
    ...
    u8g_sdl_color[0] = 0;
    u8g_sdl_color[1] = 0xff0000;
    u8g_sdl_color[2] = 0x00ff00;
    u8g_sdl_color[3] = 0x0000ff;
    ...
    */
    ...
    //   SDL_UpdateRect(u8g_sdl_screen, 0,0,u8g_sdl_width,height);
    monitor_init();
...
static const u8x8_display_info_t u8x8_sdl_128x64_info =
    {
        ...
        /* tile_width = */ 16,
        /* tile_hight = */ LV_VER_RES_MAX / 8,
        /* default_x_offset = */ 0,
        /* flipmode_x_offset = */ 0,
        /* pixel_width = */ 128,
        /* pixel_height = */ LV_VER_RES_MAX};
...
uint8_t u8x8_d_sdl_128x64(u8x8_t *u8g2, uint8_t msg, uint8_t arg_int, void *arg_ptr)
{
    ...
    case U8X8_MSG_DISPLAY_SETUP_MEMORY:
        ...
        u8g_sdl_init(128, LV_VER_RES_MAX);
        break;
...
void u8g2_SetupBuffer_SDL_128x64(u8g2_t *u8g2, const u8g2_cb_t *u8g2_cb)
{
    ...
    static uint8_t buf[128 * LV_VER_RES_MAX / 8];

    u8x8_Setup_SDL_128x64(u8g2_GetU8x8(u8g2));
    u8g2_SetupBuffer(u8g2, buf, LV_VER_RES_MAX / 8, u8g2_ll_hvline_vertical_top_lsb, u8g2_cb);
}
