...
/*
  WouoUI - wave 是 WouoUI 通用版本的分支，灵感来自 macOS 程序坞的波浪效果
...
// 绘制外框
void list_draw_check_box_frame() { u8g2.drawRFrame(CHECK_BOX_L_S + list.curve, list.temp + CHECK_BOX_U_S, CHECK_BOX_F_W, CHECK_BOX_F_H, 1); }

// 绘制框里面的点
void list_draw_check_box_dot() { u8g2.drawBox(CHECK_BOX_L_S + CHECK_BOX_D_S + 1 + list.curve, list.temp + CHECK_BOX_U_S + CHECK_BOX_D_S + 1, CHECK_BOX_F_W - (CHECK_BOX_D_S + 1) * 2, CHECK_BOX_F_H - (CHECK_BOX_D_S + 1) * 2); }
...
{
    u8g2.drawStr(LIST_TEXT_S + list.curve, list.temp + LIST_TEXT_H + LIST_TEXT_S, arr[i].m_select);
...
void list_show(struct MENU arr[], uint8_t ui_index)
{
    ...
    if (!ui.init)
    {
        for (int i = 0; i < ui.num[ui_index]; ++i)
        {
            ...
            list.curve = (ui.param[LIST_CUR] / 1000.0) * pow(list.temp - list.box_y, 2);
