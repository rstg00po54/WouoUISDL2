...
struct
{
    ...
    int16_t up_s = (DISP_H - LIST_LINE_H) / 2;
    int16_t temp;
    bool loop;
    float curve;
    float y;
...
/*
//默认参数
#define   CHECK_BOX_L_S       105                         //选择框在每行的左边距
...
void ui_param_init()
{
    ...
    ui.param[LIST_UFD] = 0;   // 菜单列表从头展开开关
...
// 绘制外框
void list_draw_check_box_frame() { u8g2.drawRFrame(CHECK_BOX_L_S + list.curve, list.up_s + list.temp + CHECK_BOX_U_S, CHECK_BOX_F_W, CHECK_BOX_F_H, 1); }

// 绘制框里面的点
void list_draw_check_box_dot() { u8g2.drawBox(CHECK_BOX_L_S + CHECK_BOX_D_S + 1 + list.curve, list.up_s + list.temp + CHECK_BOX_U_S + CHECK_BOX_D_S + 1, CHECK_BOX_F_W - (CHECK_BOX_D_S + 1) * 2, CHECK_BOX_F_H - (CHECK_BOX_D_S + 1) * 2); }
...
{
    u8g2.drawStr(LIST_TEXT_S + list.curve, list.up_s + list.temp + LIST_TEXT_H + LIST_TEXT_S, arr[i].m_select);
    u8g2.setCursor(CHECK_BOX_L_S + list.curve, list.up_s + list.temp + LIST_TEXT_H + LIST_TEXT_S);
...
void list_show(struct MENU arr[], uint8_t ui_index)
{
    ...
    // 检查循环动画是否结束
    if (list.loop && list.y == list.y_trg)
        list.loop = false;
    ...
    if (!ui.init)
    {
        for (int i = 0; i < ui.num[ui_index]; ++i)
        {
            if (ui.param[LIST_UFD])
                ...
                list.temp = (i - ui.select[ui.layer]) * list.y;
        ...
        {
            ui.init = true;
            list.y = list.y_trg = -LIST_LINE_H * ui.select[ui.layer];
        }
    }
    else
        for (int i = 0; i < ui.num[ui_index]; ++i)
    ...
    u8g2.setDrawColor(2);
    u8g2.drawRBox(0, list.up_s, list.box_x, LIST_LINE_H, LIST_BOX_R);
        ...
        case BTN_ID_CC:
            if (ui.select[ui.layer] == 0)
            {
                if (ui.param[LIST_LOOP] && ui.init)
                {
                    ...
                    list.y_trg = -(ui.num[ui.index] - 1) * LIST_LINE_H;
                    break;
                }
                else
                    break;
    ...
    if (btn.pressed)
    {
        ...
        case BTN_ID_SP:
            ...
            case 1:
                ...
                break;
            case 2:
                window_value_init("List Cur", LIST_CUR, &ui.param[LIST_CUR], 100, 0, 1, setting_menu, M_SETTING);
                break;
            case 3:
...
void oled_init()
{
    u8g2.setBusClock(20000000); // 硬件IIC接口使用，黑屏降低数值
    // u8g2.setBusClock(1000000);  //硬件IIC接口使用，黑屏降低数值
    u8g2.begin();
    u8g2.setContrast(ui.param[DISP_BRI]);
