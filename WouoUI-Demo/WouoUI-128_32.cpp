...
PROGMEM const uint8_t main_icon_pic[][16 * 7]{
    {
        0xFF,
        0xFF,
        0xFF,
...
// 磁贴变量
// 没有定义文字列表的磁贴类，需要定义图标数量
#define MAIN_ICON_NUMB 4
// 所有磁贴页面都使用同一套参数
#define TILE_ICON_H 28 // 磁贴图标高度
#define TILE_ICON_W 28 // 磁贴图标宽度
#define TILE_ICON_S 32 // 磁贴图标间距
#define TILE_INDI_H 2  // 磁贴大标题指示器高度
#define TILE_INDI_W 14 // 磁贴大标题指示器宽度
struct
{
    int16_t temp;
    bool select_flag;
...
void ui_init()
{
    ui.num[M_MAIN] = MAIN_ICON_NUMB; // 没有文字的磁贴类要在这里添加该页面图标数量
...
// 磁贴类页面通用显示函数
void tile_show(const uint8_t icon_pic[][16 * 7])
{
    // 计算动画过渡值
    ...
    // 绘制图标指示器
    u8g2.drawBox((DISP_W - tile.indi_x) / 2, DISP_H - TILE_INDI_H, tile.indi_x, TILE_INDI_H);
